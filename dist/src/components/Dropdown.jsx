...
const LabelElement = ({ onClick, label, }) => {
    if (!label)
        return null;
    if (typeof label === "string")
        return (<label className={`cursor-pointer`} onClick={onClick}>
        {label}
      </label>);
...
const DropdownMenu = ({ children, className, makClassName, liClassName, liMakClassName, }) => {
    ...
    const isSelect = (option) => {
        if (typeof option === "string" && typeof value === "string") {
            ...
            return (option.label.toLowerCase() === (value === null || value === void 0 ? void 0 : value.toString().toLowerCase()) ||
                option.value.toLowerCase() === (value === null || value === void 0 ? void 0 : value.toString().toLowerCase()));
        }
        return false;
    };
    return (<mak.div makClassName={makClassName || "bg-dark-800"} className={className || "p-2 rounded-md drop-shadow-sm text-sm"}>
        ...
        {!children && (<>
            <ul className="flex flex-col gap-2">
              {options === null || options === void 0 ? void 0 : options.map((option, i) => {
                ...
                return (<mak.li key={i} className={className ||
                        "flex cursor-pointer select-none items-center space-x-2 text-sm font-semibold"} makClassName={makClassName || "text-primary hover:text-bg|secondary-300"}>
                    <mak.span className={`w-full rounded-md px-4 py-1.5 fade-in-out`} 
                //hover:bg-${
...
const DropdownTrigger = ({ icon, label, labelLeft, labelRight, chevronRight, chevronLeft, }) => {
    ...
    return (<div className="relative flex w-fit select-none" ref={triggerRef}>
      <div onClick={() => {
            setDropdownOpen(!dropdownOpen);
        }} className="relative flex h-fit items-center justify-center gap-1">
        {chevronLeft && (<mak.span className="flex items-center" motion={{
                initial: { rotate: 0 },
        ...
        {label && labelLeft && (<LabelElement onClick={() => {
                setDropdownOpen(!dropdownOpen);
            }} label={label}/>)}
        <span className="flex cursor-pointer items-center gap-1">{icon}</span>
        {label && labelRight && (<LabelElement onClick={() => {
                setDropdownOpen(!dropdownOpen);
            }} label={label}/>)}
        {chevronRight && (<motion.span initial={{ rotate: 0 }} animate={{ rotate: dropdownOpen ? 180 : 0 }} transition={{ duration: 0.2 }} className="flex items-center">
            <mak.span makClassName="text-primary">
              <BiChevronUp className={`size-4`}/>
            </mak.span>
          </motion.span>)}
...
const DropdownComponent = ({ children, options, value, values, onChange, menuPosition = "bottom-right", icon, label, labelLeft, labelRight, chevronRight, chevronLeft, valueKey, }) => {
    ...
    return (<DropdownContext.Provider value={{
      ...
      <DropdownTrigger icon={icon} label={label} labelLeft={labelLeft} labelRight={labelRight} chevronLeft={chevronLeft} chevronRight={chevronRight}/>
      <AnimatePresence>
        {dropdownOpen && (<mak.span className={`fixed z-30 flex w-fit p-2 h-full overflow-hidden rounded-lg`} makClassName="bg-dark-800" motion={{
                variants: menuVariants,
                initial: "hidden",
                animate: dropdownOpen ? "visible" : "exit",
                exit: "exit",
                style: position,
            }} ref={dropdownRef} key={`dropdown`}>
            {children ? children : <DropdownMenu />}
          </mak.span>)}
      </AnimatePresence>
      <div className="fixed left-[-9999px] top-[-9999px] z-[-1000] flex w-fit p-2 overflow-hidden rounded-lg" ref={hiddenDropdownRef} key={"dropdown-hidden"}>
        {children && (<DropdownMenu key="dropdown-children">{children}</DropdownMenu>)}
        {options && !children && <DropdownMenu key={"dropdown-options"}/>}
      </div>
    </DropdownContext.Provider>);
};
const Dropdown = (props) => {
    return (<ComponentWrapper {...props} type="dropdown">
      {(computedProps) => {
            return (<DropdownComponent {...computedProps} options={props.options} value={props.value} values={props.values} onChange={props.onChange} menuPosition={props.menuPosition} icon={props.icon} label={props.label} labelLeft={props.labelLeft} labelRight={props.labelRight} chevronLeft={props.chevronLeft} chevronRight={props.chevronRight} dismissOnClick={props.dismissOnClick} valueKey={props.valueKey}/>);
        }}
    </ComponentWrapper>);
};
